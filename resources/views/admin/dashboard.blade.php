...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
        ...
        <div class="bg-white p-6 rounded-xl shadow flex items-center gap-4">
            ...
            <div>
                <p class="text-sm text-gray-500 font-medium">Total Pharmacies</p>
    ...
    <div class="mb-12">
        <div class="flex items-center gap-2 mb-4">
            ...
            <h2 class="text-xl font-bold text-gray-800">Users with Pharmacies</h2>
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase">Pharmacy</th>
                    ...
                    <tr class="hover:bg-blue-50 transition">
                        ...
                        <td class="px-6 py-3">
                            <form action="{{ route('admin.users.destroy', $user->id) }}" method="POST" onsubmit="return confirm('Delete this user?');">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded shadow">Delete</button>
                            </form>
                        </td>
                    </tr>
                    @empty
                    <tr><td colspan="5" class="py-4 text-center text-gray-400">No users with pharmacies found.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    ...
    <div class="mb-12">
        <div class="flex items-center gap-2 mb-4">
            ...
            <h2 class="text-xl font-bold text-gray-800">Regular Users</h2>
        </div>
        <div class="overflow-x-auto rounded-xl shadow">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    ...
                    @empty
                    <tr><td colspan="4" class="py-4 text-center text-gray-400">No regular users found.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="py-2 px-4 flex gap-2">
                        <form action="{{ route('admin.pharmacies.destroy', $pharmacy->id) }}" method="POST" onsubmit="return confirm('Delete this pharmacy?');">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="bg-red-500 text-white px-3 py-1 rounded">Delete</button>
                        </form>
                        <a href="https://www.google.com/maps/search/?api=1&query={{ $pharmacy->latitude }},{{ $pharmacy->longitude }}" target="_blank" class="bg-green-500 text-white px-3 py-1 rounded">View Map</a>
                        <a href="{{ route('admin.pharmacies.details', $pharmacy->id) }}" class="bg-blue-600 text-white px-3 py-1 rounded">Details</a>
                    </td>
                </tr>
                @endforeach
            </tbody>
